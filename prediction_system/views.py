    ...
    def post(self,request):
        try:
            if request.META["HTTP_OCP_APIM_SUBSCRIPTION_KEY"] == api_key:
                ...
                result = loaded_model.predict(x_pred)
                classifier = pickle.load(open('furniture/furnitureClassmodel.sav', 'rb'))
                importance = classifier.predict(x_pred)
                dict = {'time1':round(result[0]),
                        'time2':max([1,round(result[0]/1.5)]),
                        'time3':max([1,round(result[0]/2)]),
    ...
    def post(self,request):
        try:
            if request.META["HTTP_OCP_APIM_SUBSCRIPTION_KEY"] == api_key:
                ...
                classifier = pickle.load(open('Electricity/ElectricityClassmodel.sav', 'rb'))
                result = loaded_model.predict(x_pred)
                importance = classifier.predict(x_pred)
                dict = {'time1':round(result[0]),
                        'time2':max([1,round(result[0]/1.5)]),
    ...
    def post(self,request):
        try:
            if request.META["HTTP_OCP_APIM_SUBSCRIPTION_KEY"] == api_key:
                ...
                result = loaded_model.predict(x_pred)
                classifier = pickle.load(open('WIFI/WIFIClassmodel.sav', 'rb'))
                importance = classifier.predict(x_pred)
                dict = {'time1':round(result[0]),
                        'time2':max([1,round(result[0]/1.5)]),
                        'time3':max([1,round(result[0]/2)]),
    ...
    def post(self,request):
        try:
            if request.META["HTTP_OCP_APIM_SUBSCRIPTION_KEY"] == api_key:
                ...
                result = loaded_model.predict(x_pred)
                classifier = pickle.load(open('Roads/RoadsClassmodel.sav', 'rb'))
                importance = classifier.predict(x_pred)
                dict = {'time1':round(result[0]),
                        'time2':max([1,round(result[0]/1.5)]),
                        'time3':max([1,round(result[0]/2)]),
    ...
    def post(self,request):
        try:
            if request.META["HTTP_OCP_APIM_SUBSCRIPTION_KEY"] == api_key:
                ...
                result = loaded_model.predict(x_pred)
                classifier = pickle.load(open('FeePay/FeePayClassmodel.sav', 'rb'))
                importance = classifier.predict(x_pred)
                dict = {'time1':round(result[0]),
                        'time2':max([1,round(result[0]/1.5)]),
                        'time3':max([1,round(result[0]/2)]),
